...
class WriterGUI:
    ...
    def get_compiled_graph(self, memory: SqliteSaver):
    ...
    def run_agent(self, start, topic, stop_after):
        ...
        with SqliteSaver.from_conn_string(":memory:") as memory:
            ...
            while self.iterations[self.thread_id] < self.max_iterations:
                self.response = compiled_graph.invoke(config, self.thread)
        ...
        with gr.Blocks(
            ...
            with gr.Tab("Agent"):
                with gr.Row():
                    topic_bx = gr.Textbox(
                        label="Essay Topic",
                        value="Why Neil Breen is the greatest film artist of all time.",
                    )
                    gen_btn = gr.Button(
                ...
                with gr.Accordion("Manage Agent", open=False):
                    with SqliteSaver.from_conn_string(":memory:") as memory:
                        compiled_graph = self.get_compiled_graph(memory)
                        checks = list(compiled_graph.nodes.keys())
                ...
                gen_btn.click(
                    ...
                    outputs=[live],
                    show_progress=True,
                ).then(fn=update_display, inputs=None, outputs=sdisps).then(
            ...
            with gr.Tab("Plan"):
                ...
                modify_btn.click(
                    ...
                    outputs=None,
                ).then(fn=update_display, inputs=None, outputs=sdisps)
